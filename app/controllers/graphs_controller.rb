...
class GraphsController < ApplicationController
    ...
    def recent_assigned_to_changes_graph
        ...
        sql << " where journalized_type = 'issue' and prop_key = 'assigned_to_id' and #{ActiveRecord::Base.connection.quote(yesterday)} <= j.created_on"
        ...
        @users = User.where("id IN ("+user_ids.join(',')+")").index_by { |user| user.id } unless user_ids.empty?
    ...
    def recent_status_changes_graph
        lastweek = (Time.now - 7.day).strftime('%Y-%m-%d %H:%M:%S')
        # Get the top visible projects by issue count
        ...
        sql << " where journalized_type = 'issue' and prop_key = 'status_id' and  #{ActiveRecord::Base.connection.quote(lastweek)} <= created_on"
    ...
    def issue_growth_graph
      ...
      ActiveRecord::Base.connection_pool.with_connection do |conn|
        ...
        sql = "SELECT project_id, #{Issue.table_name}.created_on as date, COUNT(*) as issue_count"
        sql << " FROM #{Issue.table_name}"
        sql << " LEFT JOIN #{Project.table_name} ON #{Issue.table_name}.project_id = #{Project.table_name}.id"
        ...
        sql << " GROUP BY project_id, #{Issue.table_name}.created_on"
    ...
    def bug_growth_graph
        ...
        bug_by_updated_on = @bugs.to_a.delete_if {|issue| !issue.closed? }.group_by {|issue| issue.updated_on.to_date }.sort
